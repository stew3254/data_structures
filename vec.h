...
static inline void vec_del_stack(vec *v) { vec_del(v, stack_del); }
// Will free elements before removing the nodes and vec
static inline void vec_del_heap(vec *v) { vec_del(v, heap_del); }
...
// Must be of the same type if you want this to work correctly
static inline vec *vec_concat(vec *v1, vec* v2) { return vec_concat_with(v1, v2, return_elem); }
